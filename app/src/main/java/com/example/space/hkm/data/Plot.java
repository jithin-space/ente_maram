...
import com.example.space.hkm.helpers.DateButtonListener;
...
//    }
//
    public ArrayList<String> getLastUpdatedBy() throws JSONException {

        JSONArray array = data.getJSONArray("recent_activity");
        JSONArray second= array.getJSONArray(0);
        JSONObject lastUser = second.getJSONObject(0);
        String time= second.getString(1);
        ArrayList<String> output= new ArrayList<String>();

//        JSONObject lastUser = data.getJSONArray("recent_activity").getJSONObject(0);
        if (lastUser != null) {
//            return[lastUser.getString("username"), DateButtonListener.formatTimestampForDisplay(time)];
            output.add(lastUser.getString("username"));
            output.add(DateButtonListener.formatTimestampForDisplay(time));
            return output;
        }
        return output;
    }
